...
FilterYaw.Filter(yaw);
  ...
  n0 = map(n0, -90, 90, 0, 127);

  if (n0 != previousn0) {
    usbMIDI.sendNoteOn(n0, 100, channel);
    delay(10);
    usbMIDI.sendNoteOff(n0, 100, channel);

    previousn0 = n0;
  }

  m0 = FilterPitch.Current();
  m0 = map(m0, -90, 90, 0, 127);

  if (m0 != previousm0) {
    usbMIDI.sendNoteOn(m0 + 12, 100, channel);
    delay(10);
    usbMIDI.sendNoteOff(m0 + 12, 100, channel);
  ...
  if (debug) {
    Serial.print ("pitch "  + String( FilterPitch.Current()) );
    Serial.print (" , ");
    Serial.print("roll " + String( FilterRoll.Current()));
    Serial.print (" , ");
    Serial.println("yaw " + String( FilterYaw.Current() ));
  }
  while (usbMIDI.read()); // read and discard any incoming MIDI messages
  delay(10);
}
